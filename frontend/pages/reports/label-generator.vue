<script setup lang="ts">
  ...
  function getItem(n: number, item: { assetId: string; name: string; parent: { name: string } } | null): LabelData {
  ...
  const items = computed(() => {
    ...
    for (let i = displayProperties.assetRange - 1; i < displayProperties.assetRangeMax - 1; i++) {
      ...
      if (item?.parent) {
        items.push(getItem(i, item as { assetId: string; parent: { name: string }; name: string }));
      } else {
